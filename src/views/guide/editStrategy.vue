      ...
      <el-col :span="16">
        <el-form :model="formData" :rules="rules" ref="formData" label-position="left"
          ...
          <el-form-item label="预算" >
            <el-input v-model="formData.trbudget"></el-input>
...
<script>
  ...
  export default {
    ...
    computed: {
      // 上传图片
      uploadUrl() {
        return this.$api.upload_file + getStore('token') + '&type=raiders'
      },
    },
    ...
    methods: {
        ...
        this.$http.get(this.$api.get_travelrecord, {
          ...
          res => {
            ...
            if (res.data.status == 200) {
              ...
              for(let i in this.formData.trproducts){
                recommend.push({name:this.formData.trproducts[i],active:true});
              }
      ...
      postData(status){
        ...
        this.formData.trproducts = [].concat(recommend);
        ...
        this.$http.post(this.$api.add_travelrecord, this.formData).then(res => {
          if (res.data.status == 200) {
            if(status == 0){
              ...
              this.$router.push('/guide/strategy')
            }else if(status == 1){
              this.$notify({
                title: '发布成功',
